"use client";

import { useLanguage } from "@/context/LanguageContext";

export default function WhatsAppButton() {
  const { language } = useLanguage();
  const whatsappUrl = `https://wa.me/9779841370260?text=${encodeURIComponent(
    language === "en"
      ? "Hello! I'd like to inquire about your products and request a free quotation."
      : "नमस्ते! म तपाईंका उत्पादनहरूको बारेमा जान्न र निःशुल्क उद्धरण अनुरोध गर्न चाहन्छु।"
  )}`;

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600&display=swap');

        .wa-btn-wrapper {
          position: fixed;
          bottom: 96px;
          right: 20px;
          z-index: 50;
          font-family: 'DM Sans', sans-serif;
        }

        .wa-btn {
          display: flex;
          align-items: center;
          gap: 10px;
          background: linear-gradient(135deg, #25D366 0%, #1aad54 100%);
          color: #fff;
          text-decoration: none;
          padding: 13px 20px 13px 16px;
          border-radius: 50px;
          box-shadow:
            0 4px 20px rgba(37, 211, 102, 0.45),
            0 1px 4px rgba(0,0,0,0.12);
          transition: all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1);
          position: relative;
          overflow: hidden;
          white-space: nowrap;
        }

        /* Shine sweep */
        .wa-btn::before {
          content: '';
          position: absolute;
          top: 0; left: -75%;
          width: 50%;
          height: 100%;
          background: linear-gradient(
            120deg,
            transparent 0%,
            rgba(255,255,255,0.25) 50%,
            transparent 100%
          );
          transform: skewX(-20deg);
          transition: left 0.6s ease;
        }
        .wa-btn:hover::before {
          left: 130%;
        }

        .wa-btn:hover {
          transform: translateY(-3px) scale(1.04);
          box-shadow:
            0 8px 30px rgba(37, 211, 102, 0.55),
            0 2px 8px rgba(0,0,0,0.15);
        }

        .wa-btn:active {
          transform: translateY(-1px) scale(1.01);
        }

        /* Ripple ring pulse */
        .wa-pulse {
          position: absolute;
          inset: 0;
          border-radius: 50px;
          animation: waPulse 2.4s ease-out infinite;
          pointer-events: none;
        }

        @keyframes waPulse {
          0%   { box-shadow: 0 0 0 0   rgba(37,211,102,0.55); }
          60%  { box-shadow: 0 0 0 14px rgba(37,211,102,0); }
          100% { box-shadow: 0 0 0 14px rgba(37,211,102,0); }
        }

        /* Icon container */
        .wa-icon {
          display: flex;
          align-items: center;
          justify-content: center;
          width: 30px;
          height: 30px;
          background: rgba(255,255,255,0.18);
          border-radius: 50%;
          flex-shrink: 0;
          backdrop-filter: blur(4px);
          transition: background 0.3s ease;
        }
        .wa-btn:hover .wa-icon {
          background: rgba(255,255,255,0.28);
        }

        /* Label */
        .wa-label {
          font-size: 13.5px;
          font-weight: 600;
          letter-spacing: 0.01em;
          line-height: 1;
        }

        /* Tooltip "Free Quote" badge */
        .wa-badge {
          position: absolute;
          top: -10px;
          right: -4px;
          background: #fff;
          color: #1aad54;
          font-size: 9.5px;
          font-weight: 700;
          letter-spacing: 0.04em;
          padding: 2px 7px;
          border-radius: 20px;
          box-shadow: 0 2px 8px rgba(0,0,0,0.12);
          text-transform: uppercase;
          pointer-events: none;
          animation: badgePop 0.4s cubic-bezier(0.34,1.56,0.64,1) both;
        }
        @keyframes badgePop {
          from { opacity: 0; transform: scale(0.6) translateY(4px); }
          to   { opacity: 1; transform: scale(1) translateY(0); }
        }
      `}</style>

      <div className="wa-btn-wrapper">
        <a
          href={whatsappUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="wa-btn"
          aria-label="Chat on WhatsApp"
        >
          <span className="wa-pulse" aria-hidden="true" />

          <span className="wa-icon">
            <svg width="17" height="17" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
          </span>

          <span className="wa-label">
            {language === "en" ? "WhatsApp Us" : "ह्वाट्सएप"}
          </span>
        </a>

        {/* Free quote badge — shown always, remove if not needed */}
        <span className="wa-badge" aria-hidden="true">
          {language === "en" ? "Free Quote" : "निःशुल्क"}
        </span>
      </div>
    </>
  );
}